...
> 本目录为同仓中的 Android 工程根。请用 **Android Studio 打开 `AIDA64Remote-Android/`**，不要打开仓库根目录。
...
1. 用 Android Studio 打开本目录（`AIDA64Remote-Android`），连接真机或模拟器后运行。
...
Windows 可使用 `gradlew.bat`。若从仓库根目录操作：
